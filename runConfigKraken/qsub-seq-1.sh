...
function submit_sequence()
{
  ...
  do
    ...
    # set variables to pass to the batch script
    ...
    # provide status at the calling term
    echo "$CONFIG_FILE $LOG_FILE $i $JID_DEP $WORK_DIR" 1>&2

    # launch the run
    JID_DEP=`qsub -j oe -M burlen.loring@gmail.com -m ae -N $JOB_NAME-$i -l size=$i,walltime=02:10:00 -v $V -W depend=afterok:$JID_DEP $BATCH_SCRIPT`
    if [[ -z "$JID_DEP" ]]
    then
